...
namespace LootLockerDemoApp
{
    public class CharacterPrefabClass : MonoBehaviour
    {
        ...
        public void Init(LootLockerCharacter character, LootLockerCharacter_Types[] characterClassTypes, InputPopupCharacter inputPopupCharacter, bool isActive)
        {
            ...
            string name = this.character != null && !string.IsNullOrEmpty(this.character.name) ? this.character.name : "My First Character";
            characterName.text = this.character != null ? name : "Empty Character Slot " + (transform.GetSiblingIndex() + 1);
        }
